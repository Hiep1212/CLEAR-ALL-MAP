...
-- Danh sách loại trừ (thu hẹp để ẩn mạnh, giữ vật phẩm cần thiết)
local excludeNames = {
...
-- Hàm ẩn object client-side (giữ va chạm server-side)
    ...
    pcall(function()
        if (obj:IsA("BasePart") or obj:IsA("MeshPart") or obj:IsA("UnionOperation")) and shouldClear(obj) then
            if obj.Parent == Workspace or obj.Parent:IsDescendantOf(Workspace) then
                obj.LocalTransparencyModifier = 1  -- Ẩn client-side
...
-- Hàm clear map (ẩn client-side, giữ va chạm server-side)
